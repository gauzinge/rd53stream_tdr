...
namespace Serializer {
    ...
    template<typename T>
        std::vector<T> toVec (std::vector<bool>& vec)
        {
            ...
            // header then size then data
            ret_vec.reserve (ret_vec_size_calc); //  ret_vec_size_calc + 1

            // put my handmade header
            ...
            //now remove the all zero words at the end
            while(ret_vec.back() == 0)
                ret_vec.pop_back();

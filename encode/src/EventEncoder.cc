...
void EncodedEvent::clear()
{
    ...
    chip_was_split.clear();
    chip_matrices.clear();
    chip_iterator = chip_matrices.begin();
}
...
void EventEncoder::get_single_module_event(EncodedEvent* encoded_event, uint32_t pEventId, ChipIdentifier pModuleId)
{
    ...
    // now get it
    return get_next_event(encoded_event, true);
}

void EventEncoder::get_next_event(EncodedEvent* encoded_event, bool pDoSingle)
{
    ...
    while (true) {
        ...
        if (!isDone) {
            ...
            //std::cout << "done filling the intMatrices" << std::endl;
